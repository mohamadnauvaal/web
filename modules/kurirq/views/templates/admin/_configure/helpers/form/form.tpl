...
{block name="input_row"}
	...
	{elseif $input.type == 'order_state'}
		{assign var=states value=$input.values}
		{if isset($states) && count($states) > 0}
						...
						<table class="table">
							<thead>
								<tr>
									...
									<th>{l s='ID' mod='kurirq'}</th>
									<th>{l s='Name' mod='kurirq'}</th>
								</tr>
							</thead>
							<tbody>
								{foreach $states as $key => $state}
									<tr class="{if $key%2}alt_row{else}not_alt_row{/if}">
										<td>
											{assign var=id_checkbox value=$state['id']}
											<input type="checkbox" class="cmsBox" name="{$input.name}" id="{$id_checkbox}" value="{$id_checkbox}" {if isset($fields_value[$id_checkbox])}checked="checked"{/if} {if $state['aktif']==0}disabled="disabled"{/if}/>
										</td>
										<td>
											{$state['id']}
										</td>
			...
			{else}
			<p>{l s='Unable to load carrier. Please refresh this page or check your api key.' mod='kurirq'}</p>
		{/if}		
	{else}
		{$smarty.block.parent}
	{/if}

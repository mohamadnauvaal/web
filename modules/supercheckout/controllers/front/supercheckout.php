                ...
                switch (Tools::getValue('method')) {
                    ...
                    case 'loadCarriers':
                        $this->json = $this->loadCarriers(
                            ...
                            null,
                            (int) Tools::getValue('id_district'), // dodol
                            (int) Tools::getValue('id_subdistrict') // dodol                            
                        );
                        break;
                    case 'setSameInvoice':
...
//                return $response;
            ...
            if ($continue == 1 && !$this->context->cart->isVirtualCart() && ((isset($posted_data['shipping_address_value']) && $posted_data['shipping_address_value'] == 1)
                ...
                $delivery_address->id_district = (!empty($posted_data['id_district'])) ? $posted_data['id_district'] : 0; // KURIRQ
                $delivery_address->id_subdistrict = (!empty($posted_data['id_subdistrict'])) ? $posted_data['id_subdistrict'] : 0; // KURIRQ
                ...
                // $delivery_address->id_district = (int)$id_district; // dodol
                // $delivery_address->id_subdistrict = (int)$id_subdistrict; // dodol
...
//        }
    ...
    private function loadCarriers(
        ...
        $id_district = 0, // KURIRQ
        $id_subdistrict = 0
    ) {
        $old_id_address_delivery = $this->checkout_session->getIdAddressDelivery();

        $this->initCheckoutAddresses($id_country, $id_state, $postcode, $id_address_delivery, $id_district, $id_subdistrict);
